...
isCheckOrCapture = lambda board, move: board.gives_check(move) or board.is_capture(move)
...
def mvvVlaScore(board, captureOrCheck):
  score = 0

  ## Checks are more valuble than just captures
  if board.gives_check(captureOrCheck):
    score -= 10 
  ...
  try:
    ...
    return (score + board.piece_type_at(captureOrCheck.from_square) - 
            board.piece_type_at(captureOrCheck.to_square))
  except:
    return score
...
def alphaBeta(alpha, beta, board, depth):
  ...
  return quiesenceSearch(alpha, beta, board)
...
def quiesenceSearch(alpha, beta, board):
  ...
  dangerousMoves = list(filter(lambda move: isCheckOrCapture(board, move), 
                          board.generate_legal_moves))
  for M in sorted(dangerousMoves, key = lambda move: mvvVlaScore(move)):
    board.push(M)
